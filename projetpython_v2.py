...
min_words = 15
dataset = dataset.filter(lambda x: x["text"] is not None and len(x["text"].split()) > min_words) #pour filtrer les prises de parole trop courtes

# Regroupement des partis avec une fréq < 5% (pour pallier au déséquilibre des classes)
# 1. On définit le seuil (ex: 5% du dataset)
threshold = 0.05

# 2. On calcule les fréquences pour identifier les petits partis
counts = dataset.to_pandas()["speaker_party"].value_counts(normalize=True)
petits_partis = counts[counts < threshold].index.tolist()

# 3. On remplace les noms par "AUTRE"
